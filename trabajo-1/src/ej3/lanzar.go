...
func main() {
	if len(os.Args) == 3 {
		...
		if err != nil {
			...
			//output, err := ssh.RunCommand("go run /home/a779088/cuarto/SSDD/trabajo-1/src/ej2/server.go")
			output, err := ssh.RunCommand("ls")
			fmt.Println(output)
			if err != nil {
				log.Printf("SSH run command error %v", err)
			}
			/*output, err = ssh.RunCommand("go run /home/a779088/cuarto/SSDD/trabajo-1/src/ej2/cliente.go")
			fmt.Println(output)
			if err != nil {
				log.Printf("SSH run command error %v", err)
			}*/
